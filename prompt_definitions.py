...
- 时间轴（关键历史事件年表）
- 法则体系（物理/魔法/社会规则的漏洞点）
- 修炼体系（按区域：人界/灵界/仙界等；按种族：人/魔/佛/妖/鬼等）
- 装备体系（包括装备体系、灵药/丹药体系、符箓体系、阵法体系、宠物体系等）

【社会维度】
- 权力结构断层线（可引发冲突的阶层/种族/组织矛盾）
- 文化禁忌（可被打破的禁忌及其后果）
- 经济命脉（资源争夺焦点）
- 地图体系（战争迷雾模式，地图应随主角的移动由剧情逐步揭示）
...
- 每个悬念单元需明确主角的修为等级范围和空间坐标范围，且范围适配整体小说架构
...
注意：以上所有字段（标题、定位、作用、悬念密度、伏笔设计、转折程度、主角修为、空间坐标、简述）都会被解析并用于后续章节生成，请确保每个字段都有明确的值。
